...
def menu():
    print("1-Agregar alumno a asignatura")
    print("2-Listas asignatura con alumnos")
    print("3-salir")
...
def buscar(codigo, rut):
    ...
    query = "SELECT * FROM `inscripcionesasignaturas` WHERE codigo = '"+codigo+"' AND rut_alumno = '"+rut+"'"
...
def insertar(codigo,rut_alumno):
    ...
    mycursor = mydb.cursor()
    mycursor.execute("USE colegio;")
    query = "INSERT INTO `inscripcionesasignaturas` (`codigo`, `rut_alumno`)"
...
def listar():
    ...
    mycursor = mydb.cursor()
    mycursor.execute("USE colegio;")
    query = "SELECT * FROM `inscripcionesasignaturas` WHERE 1;"
    mycursor.execute(query)
